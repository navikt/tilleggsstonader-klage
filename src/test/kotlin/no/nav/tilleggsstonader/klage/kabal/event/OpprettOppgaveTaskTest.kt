...
import no.nav.tilleggsstonader.kontrakter.oppgave.PersonIdent as OppgavePersonIdent
...
class OpprettOppgaveTaskTest : IntegrationTest() {
    ...
    @Autowired
    lateinit var personRepository: FagsakPersonRepository
    ...
    fun setup() {
        opprettOppgaveTask =
            OpprettOppgaveForKlagehendelseTask(fagsakRepository, behandlingRepository, personRepository, oppgaveService)
    ...
    fun `skal lage oppgave med riktige verdier i request`() {
        ...
        assertThat(opprettOppgaveRequestSlot.captured.personident).isEqualTo(
            OppgavePersonIdent(
                personIdent,
            ),
        )
        ...
        assertThat(opprettOppgaveRequestSlot.captured.behandlingstema).isEqualTo(Behandlingstema.PassAvBarn.value)
